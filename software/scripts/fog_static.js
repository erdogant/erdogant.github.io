...
function startFog(canvas, img) {
  ...
  function drawFog() {
    ...
    // gradient.addColorStop(0, `rgba(220,220,220,${fogDensity * 0.6})`);
    // gradient.addColorStop(1, `rgba(220,220,220,${fogDensity})`);
...
function animateFog(prefix) {
  ...
  // Add to image
  if (metar_obj.visibility && metar_obj.visibility < 2500) {
    controller.start("heavy");
  } else if (metar_obj.visibility && metar_obj.visibility < 5000) {
    controller.start("medium");
  } else if (metar_obj.visibility && metar_obj.visibility < 8000) {
