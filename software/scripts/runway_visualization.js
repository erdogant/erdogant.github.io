...
function drawRunwayBar(ctx, width, y, available, threshold, ft50, thresholdOK, ft50OK) {
  ...
  ctx.fillStyle = thresholdOK ? "rgba(76, 175, 80, 0.5)" : "rgba(244, 67, 54, 0.5)";
  ctx.fillRect(margin, y + 5, thresholdWidth, 20);

  // Draw 50ft required
  const ft50Width = Math.min(ft50 * scale, barWidth);
  ctx.fillStyle = ft50OK ? "rgba(33, 150, 243, 0.5)" : "rgba(255, 152, 0, 0.5)";
...
/**
 * Create a simple text-based results display (alternative to canvas)
 */
function displayRunwayResultsDiv(prefix) {
  ...
  container50ft.innerHTML = `
    <div style="text-align: center; padding: 10px; background: white; border-radius: 4px; border: 2px solid ${ft50OK ? "#2196F3" : "#FF9800"};">
      <div style="font-size: 12px; color: #666;">50ft Required</div>
      <div style="font-size: 24px; font-weight: bold; color: ${ft50OK ? "#2196F3" : "#FF9800"};">${required50ft}m</div>
    ...
    <div style="margin-bottom: 20px; background: transparent; padding: 20px; border-radius: 4px;">
      <div style="position: relative; height: 120px; background: #1a1a1a; border-radius: 2px; overflow: visible; box-shadow: 0 4px 8px rgba(0,0,0,0.3);">
        ...
        ${(() => {
          ...
          const runwayColor = surfaceType === "soft" ? "#4a7c4e" : "#2a2a2a";
        ...
        <div style="position: absolute; left: 5%; top: 10%; width: ${Math.min((requiredThreshold / availableLength) * 100 - 5, 95)}%; height: 30%; background: ${thresholdOK ? "rgba(76, 175, 80, 0.4)" : "rgba(244, 67, 54, 0.4)"}; border-right: 3px solid ${thresholdOK ? "#4CAF50" : "#F44336"};">
        ...
        <div style="position: absolute; left: 5%; bottom: 10%; width: ${Math.min((required50ft / availableLength) * 100 - 5, 95)}%; height: 30%; background: ${ft50OK ? "rgba(33, 150, 243, 0.4)" : "rgba(255, 152, 0, 0.4)"}; border-right: 3px solid ${ft50OK ? "#2196F3" : "#FF9800"};">
      ...
      <div style="display: flex; gap: 15px; margin-top: 40px; flex-wrap: wrap; justify-content: center;">
        <div style="display: flex; align-items: center; gap: 5px;">
          <div style="width: 20px; height: 12px; background: ${thresholdOK ? "rgba(76, 175, 80, 0.6)" : "rgba(244, 67, 54, 0.6)"}; border: 1px solid ${thresholdOK ? "#4CAF50" : "#F44336"};"></div>
          <span style="font-size: 11px; color: black;">Threshold: ${requiredThreshold}m ${thresholdOK ? "✓" : "✗"}</span>
        </div>
        <div style="display: flex; align-items: center; gap: 5px;">
          <div style="width: 20px; height: 12px; background: ${ft50OK ? "rgba(33, 150, 243, 0.6)" : "rgba(255, 152, 0, 0.6)"}; border: 1px solid ${ft50OK ? "#2196F3" : "#FF9800"};"></div>
  ...
  // containerBar.innerHTML = `
  //   <!-- Realistic Runway Visualization -->
  //   <div style="margin-bottom: 20px; background: #2a5f2f; padding: 20px; border-radius: 4px;">
  //     <div style="position: relative; height: 120px; background: #1a1a1a; border-radius: 2px; overflow: visible; box-shadow: 0 4px 8px rgba(0,0,0,0.3);">

  //       <!-- Runway surface with grass edges -->
  //       <div style="position: absolute; top: -10px; left: -10px; right: -10px; bottom: -10px; background: #2a5f2f;"></div>

  //       <!-- Main runway surface (asphalt) -->
  //       <div style="position: absolute; left: 0; top: 0; width: 100%; height: 100%; background: #2a2a2a;"></div>
  ...
  //       <!-- Threshold required distance indicator -->
  //       <div style="position: absolute; left: 0; top: 0; width: ${Math.min((requiredThreshold / availableLength) * 100, 100)}%; height: 35%; background: ${thresholdOK ? "rgba(76, 175, 80, 0.4)" : "rgba(244, 67, 54, 0.4)"}; border-right: 3px solid ${thresholdOK ? "#4CAF50" : "#F44336"};">
  //         <span style="position: absolute; right: 5px; top: 5px; font-size: 10px; color: white; font-weight: bold; background: rgba(0,0,0,0.6); padding: 2px 6px; border-radius: 2px;">THRESHOLD</span>
  //       </div>

  //       <!-- 50ft required distance indicator -->
  //       <div style="position: absolute; left: 0; bottom: 0; width: ${Math.min((required50ft / availableLength) * 100, 100)}%; height: 35%; background: ${ft50OK ? "rgba(33, 150, 243, 0.4)" : "rgba(255, 152, 0, 0.4)"}; border-right: 3px solid ${ft50OK ? "#2196F3" : "#FF9800"};">
  //         <span style="position: absolute; right: 5px; bottom: 5px; font-size: 10px; color: white; font-weight: bold; background: rgba(0,0,0,0.6); padding: 2px 6px; border-radius: 2px;">50FT</span>
  //       </div>

  //       <!-- Distance markers below runway -->
  //       <div style="position: absolute; top: 100%; left: 0; width: 100%; height: 30px; display: flex; justify-content: space-between; padding-top: 5px;">
  //         <span style="font-size: 10px; color: white;">0m</span>
  //         <span style="font-size: 10px; color: white;">${Math.round(availableLength / 4)}m</span>
  //         <span style="font-size: 10px; color: white;">${Math.round(availableLength / 2)}m</span>
  //         <span style="font-size: 10px; color: white;">${Math.round((3 * availableLength) / 4)}m</span>
  //         <span style="font-size: 10px; color: white;">${availableLength}m</span>
  //       </div>
  //     </div>
  ...
  //       <div style="display: flex; align-items: center; gap: 5px;">
  //         <div style="width: 20px; height: 12px; background: ${thresholdOK ? "rgba(76, 175, 80, 0.6)" : "rgba(244, 67, 54, 0.6)"}; border: 1px solid ${thresholdOK ? "#4CAF50" : "#F44336"};"></div>
  //         <span style="font-size: 11px; color: white;">Threshold: ${requiredThreshold}m ${thresholdOK ? "✓" : "✗"}</span>
  //       </div>
  //       <div style="display: flex; align-items: center; gap: 5px;">
  //         <div style="width: 20px; height: 12px; background: ${ft50OK ? "rgba(33, 150, 243, 0.6)" : "rgba(255, 152, 0, 0.6)"}; border: 1px solid ${ft50OK ? "#2196F3" : "#FF9800"};"></div>
  //         <span style="font-size: 11px; color: white;">50ft: ${required50ft}m ${ft50OK ? "✓" : "✗"}</span>
  //       </div>
  //       <div style="display: flex; align-items: center; gap: 5px;">
  //         <div style="width: 20px; height: 12px; background: #2a2a2a; border: 1px solid white;"></div>
  //         <span style="font-size: 11px; color: white;">Available: ${availableLength}m</span>
  //       </div>
  //     </div>
  //   </div>
  //   `;
  ...
  //     <div style="position: relative; height: 60px; background: #e0e0e0; border-radius: 4px; overflow: hidden;">
  //       <!-- Available runway -->
  //       <div style="position: absolute; left: 0; top: 0; width: 100%; height: 100%; background: #e0e0e0; border: 2px solid #999;"></div>

  //       <!-- Threshold required -->
  //       <div style="position: absolute; left: 0; top: 5px; width: ${Math.min((requiredThreshold / availableLength) * 100, 100)}%; height: 20px; background: ${thresholdOK ? "rgba(76, 175, 80, 0.7)" : "rgba(244, 67, 54, 0.7)"}; border-radius: 2px;">
  //         <span style="position: absolute; right: 5px; top: 2px; font-size: 11px; color: white; font-weight: bold;">Threshold</span>
  //       </div>

  //       <!-- 50ft required -->
  //       <div style="position: absolute; left: 0; top: 35px; width: ${Math.min((required50ft / availableLength) * 100, 100)}%; height: 20px; background: ${ft50OK ? "rgba(33, 150, 243, 0.7)" : "rgba(255, 152, 0, 0.7)"}; border-radius: 2px;">
  //         <span style="position: absolute; right: 5px; top: 2px; font-size: 11px; color: white; font-weight: bold;">50ft</span>
  //       </div>
  //     </div>
  //   </div>
  //   `;
      ...
      ${
        ...
        <div style="margin-top: 15px; padding: 10px; background: #fff3cd; border: 1px solid #ffc107; border-radius: 4px;">
          ...
          <div style="color: #856404; margin-top: 5px;">
            ${!thresholdOK ? `Threshold distance exceeds available runway by ${requiredThreshold - availableLength}m.<br>` : ""}
...
function createBreakdownRows(resultThreshold, result50ft) {
  const steps = [
    { name: "POH (Altitude, Temp, Weight)", data: "POH" },
    ...
    .map((step) => {
      ...
      if (thresholdData) {
        ...
        <tr>
          ...
          <td style="padding: 8px; text-align: center; border: 1px solid #ddd; color: #2196F3; font-weight: bold;">${factor}</td>
        </tr>
      `;
      }
      return "";
 ...
 */
function enableAutoUpdateResults(prefix, containerId, useCanvas = false) {
  const updateFunc = useCanvas ? displayRunwayResults : displayRunwayResultsDiv;

  const inputIds = [
    `${prefix}_RUNWAY_LENGTH`,
    `${prefix}_RUNWAY_SLOPE`,
    `${prefix}_ELEVATION`,
    `${prefix}_TEMPERATURE`,
    `${prefix}_WIND_DIRECTION`,
    `${prefix}_WIND_STRENGTH`,
    `${prefix}_RUNWAY_SURFACE`,
    `${prefix}_RUNWAY_SLIPPERY`,
    `${prefix}_total_weight`,
  ];
  ...
  inputIds.forEach((id) => {
    ...
    if (element) {
      element.addEventListener("change", () => {
        ...
        timeout = setTimeout(() => updateFunc(prefix, containerId), 300);
      });
    }
  });

  // Initial display
  updateFunc(prefix, containerId);
}
...
window.enableAutoUpdateResults = enableAutoUpdateResults;
